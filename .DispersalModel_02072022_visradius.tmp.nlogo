...
turtles-own [natal risk dispersal-distance-size preferred-patch wetland-name age previous-patch excursion-distance]
...
to setup
  ...
  if sea-bird = 1 and patch-data = 0
...
;;;adult risk modifications to survival start - based on wetland risk. note added code for switching risk-sensitivity on and off
  ...
  ask first-breeders [
    ...
    if(risk-sensitive = 1) [  set risk ( sum [ risk-level ] of patches in-radius vision-radius - ( sum [ risk-level ] of patches in-radius 0 ) )
...
ask sub-adults [
  ...
  ;;  within their home range size. If risk is less than risk tolerance, they just keep their preferred patch the same.
   ...
   ask repeat-breeders [
    ...
    set excursion-distance distance wetland-name
     set risk ( sum [ risk-level ] of patches in-radius vision-radius - ( sum [ risk-level ] of patches in-radius 0 ) )
    ...
    if (risk < risk-threshold ) [
      set preferred-patch [ nest-patch ] of one-of patches with [ habitat-type = "nesting" ] in-radius 0
  ]
