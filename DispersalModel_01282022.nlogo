extensions [ gis ]
globals [ patch-data gis-file current-pop prev-pop ]
...
to setup
  ...
  ask turtles [
    ...
    set natal [ nest-patch ] of one-of patches with [ habitat-type = "nesting" ] in-radius 120
...
ask sub-adults [
  ...
  ;;  within their home range size. If risk is less than risk tolerance, they just keep their preferred patch the same.
   ...
   ask repeat-breeders [
    ...
    face min-one-of patches with [ nest-patch = my-nest-patch] [ distance myself ]
    fd ( dispersal-distance-size )
...
;; random probability of a risk leading to unexpected mortality during nest excursions - only included if risk-sensitive adult survival is switched on
  ...
  ;; On 03/03/2020, using numeric coding for different versions of nest success. 1= base, 2 = risk sensitive, 3 = empirical
    ...
    if nestsuccess <= nest-survival  [ ifelse count turtles <= K  [ hatch-hatchlings 8 [ move-to wetland-name set color green set shape "turtle" set age 0 set preferred-patch natal hide-turtle] ] [ hatch-hatchlings 0 [ move-to wetland-name set color green set shape "turtle" set age 0 set preferred-patch natal hide-turtle] ] ]
    ]
;; nest survival based on risk
    if (nest-success-mode = 2) [
      let nestsuccess random-float 1
      if (risk > risk-threshold) [ if nestsuccess <= (nest-survival * nest-risk-penalty)  [ ifelse count turtles <= K  [ hatch-hatchlings 8 [ move-to wetland-name set color green set shape "turtle" set age 0 set preferred-patch natal hide-turtle] ] [ hatch-hatchlings 0 [ move-to wetland-name set color green set shape "turtle" set age 0 set preferred-patch natal hide-turtle] ] ]
    ]
...
if (previous-patch = 1) [if nestsuccess <= 0  [ ifelse count turtles <= K  [ hatch-hatchlings 8 [ move-to wetland-name set color green set shape "turtle" set age 0 set preferred-patch natal hide-turtle] ] [ hatch-hatchlings 0 [ move-to wetland-name set color green set shape "turtle" set age 0 set preferred-patch natal hide-turtle] ] ]]
  ...
  ask first-breeders [
    ...
    if (natalcorrect < natal-error-rate) [
      ...
      face min-one-of patches with [ nest-patch = my-nest-patch ] [ distance myself ]
      fd ( dispersal-distance-size )
      move-to one-of patches with [ habitat-type = "nesting" ] in-radius (dispersal-distance-size)
      set preferred-patch [ nest-patch ] of one-of patches with [ habitat-type = "nesting" ] in-radius 1
  ]
...
"" ""
PENS
"breeding dispersal" 1.0 1 -2674135 true "" "if ( count repeat-breeders > 0 ) [ plot  ( count repeat-breeders with [ preferred-patch != previous-patch ] )  / ( count repeat-breeders ) ]"
"natal dispersal" 1.0 1 -2064490 true "" "if ( count first-breeders > 0 ) [ plot  ( ( count first-breeders with [ preferred-patch != natal ] ) /( count first-breeders ) ) ]"
"empirical natal dispersal" 1.0 0 -16777216 true "" "plot 0.0927"
"empirical breeding dispersal" 1.0 0 -11053225 true "" "plot 0.0111"
...
1108
450
Mean Natal Dispersal (Reid et al. 2016b)= 0.0927\nMean Breeding Dispersal (Reid et al. 2016b)= 0.0111\nRatio Natal:Breeding = 8.35
11
0.0
1
...
1049
423
Empirical Dispersal Information
14
0.0
1
